 ...
 * string
 */
export const strLimit = (text: string, limit: number) =>
  (text ?? "").slice(0, limit);

export const strTruncate = (text: string, limit = 10) =>
  strLimit(text, limit) + ((text ?? "").length > limit ? "..." : "");
 ...
 * Number
 */
export const setDigit = (number: string | number, digitLength: number) =>
  Number(number).toLocaleString("en-US", { minimumIntegerDigits: digitLength });
...
export function numberToRoman(num?: number) {
  ...
  for (const [keyRoman, valueRoman] of Object.entries(roman)) {
    const q = Math.floor(num / valueRoman);
    num -= q * valueRoman;
 ...
 * Date
 */
export const epoch10to13 = (epoch: number) =>
  Number(String(epoch).length == 10 ? Math.floor(epoch * 1000) : epoch);

export const epoch13to10 = (epoch: number) =>
  Number(String(epoch).length == 13 ? Math.floor(epoch / 1000) : epoch);
...
export function formatValueInputDate(date: any) {
  ...
  return Number.isInteger(epoch)
    ? DateTime.fromSeconds(epoch).toFormat("yyyy-MM-dd")
    : epoch;
}
...
export function formatValueInputDateTime(date: any) {
  const epoch = dateToEpoch10(date);
  return Number.isInteger(epoch)
    ? DateTime.fromSeconds(epoch).toFormat(`yyyy-MM-dd'T'mm:ss`)
    : epoch;
}
...
export const objectToQueryUrl = (params: Record<string, any>) => {
  ...
  try {
    Object.keys(params).forEach((res) => {
      if (params[res]) {
        queryPharams.push(
          encodeURIComponent(res) + "=" + encodeURIComponent(params[res]),
        );
      }
    });
