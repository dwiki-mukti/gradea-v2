...
import { CSSProperties, DetailedHTMLProps, Dispatch, HTMLAttributes, ReactNode, SetStateAction, useEffect, useState } from 'react'
import Confirm from './popups/Confirm';
import Search from './Search';
import { api } from '../utils/frontend';
import { ChevronLeftIcon, ChevronRightIcon, EllipsisHorizontalCircleIcon, EyeIcon, FunnelIcon, PencilSquareIcon, TrashIcon } from '@heroicons/react/24/solid';
                                ...
                                DataTables.dataRows?.map((dataRow: Record<string, any>, indexDataRow) => {
                                    ...
                                    return (
                                        <tr key={indexDataRow}>
                                            {!noNumber && (
                                                <td>
                                                    {((ObjectParams?.per_page ?? 10) * ((ObjectParams?.page ?? 1) > 0 ? (ObjectParams?.page ?? 1) - 1 : 0)) + indexDataRow + 1}
                                                </td>
                                            )}
        ...
        <div className='relative flex items-center'>
            ...
            >
                {[10, 25, 50, 100].map((count, indexCount) => (
                    <option key={indexCount} className='pl-0' value={count}>{count} Entries</option>
                ))}
            </select>
        </div >
    )
...
function Paginate({
    ...
    return (
        <div className='flex items-baseline'>
            <div className=''>
                <div className='text-xs'>Showing {perPage * (currentPage > 0 ? currentPage - 1 : 0)} to {showedItem} of {totalItem} entries</div>
            </div>
            <div className='ml-auto'>
                <ul className="flex flex-row items-center gap-1">
                    {(prevPage >= 1) && (
                        <PaginateItem className="px-4 py-2 gap-1 pl-2.5" onClick={() => onSwitch(prevPage)}>
                            <ChevronLeftIcon />
                            <span>Previous</span>
                        </PaginateItem>
                    )}
                    ...
                    {(nextPage < lastPage) && (
                        ...
                        >{lastPage}</PaginateItem>
                    )}
                    {(nextPage <= lastPage) && (
                        <PaginateItem className="px-4 py-2 gap-1 pr-2.5" onClick={() => onSwitch(nextPage)}>
                            <span>Next</span>
                            <ChevronRightIcon />
                        </PaginateItem>
                    )}
        ...
        <div>
            <div className='flex items-center h-[2.25rem] border border-gray-400 rounded-lg overflow-hidden'>
                ...
                <div className='cursor-default flex items-center'>
                    <div className='px-1 pt-[2px]'>{page}/{lastPage}</div>
                </div>
                <div
                    onClick={() => {
