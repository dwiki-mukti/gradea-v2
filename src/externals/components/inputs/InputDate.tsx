...
import {
  epoch10to13,
  formatValueInputDate,
  formatValueInputDateTime,
} from "@/externals/utils/general";
...
> & { type?: "date" | "datetime-local" }) {
  ...
  function sanitizeValue(newVal: any) {
    if (newVal) {
      return type == "datetime-local"
        ? formatValueInputDateTime(newVal)
        : formatValueInputDate(newVal);
    } else {
      return undefined;
    }
  }
  ...
  useEffect(() => {
    ...
    const addToUncompleteds =
      validations?.required &&
      !currentValue &&
      !getter?.uncompleteds?.includes(name);
    const removeFromUncompleteds =
      validations?.required &&
      currentValue &&
      getter?.uncompleteds?.includes(name);

    // adjust invalid
    const invalidMessages: string[] = inputValidator({
      validations,
      fieldName: name,
      formControl: getter,
    });
    const invalidMessagesIsChange =
      JSON.stringify(invalidMessages) !=
      JSON.stringify(getter?.invalids?.[name]);

    // sync value
    const isSyncPropsValue = ![undefined, getter.values?.[name]].includes(
      value,
    );
    const isSyncValueElement =
      getter.values?.[name] != undefined &&
      refInput.current?.value != (getter.values?.[name] ?? "");

    // push to state
    if (
      addToUncompleteds ||
      removeFromUncompleteds ||
      invalidMessagesIsChange ||
      isSyncPropsValue ||
      isSyncValueElement
    ) {
      setter((prev: typeStateInput) => {
        const uncompleteds = (prev?.uncompleteds ?? []).filter(
        ...
        if (refInput.current && isSyncValueElement)
          refInput.current.value = String(values[name] ?? "");
        return {
          ...(prev ?? {}),
          uncompleteds,
          values,
